...
if not isinstance(asset, unreal.LevelSequence):
    ...
    target = None
    for a in actors:
        if a.get_actor_label() == _label:
            target = a
            break
    if target is None:
        print("ERROR: Actor '" + _label + "' not found in level")
    else:
        # Use LevelSequenceEditorBlueprintLibrary if available
        try:
            binding = asset.add_possessable(target)
            if binding:
                print(f"Added {{target.get_actor_label()}} to sequence {{_asset_path}}")
            else:
...
if not isinstance(asset, unreal.LevelSequence):
    ...
    if movie_scene:
        ...
        # Try set_playback_range first
        set_ok = False
        if hasattr(movie_scene, 'set_playback_range'):
            try:
                movie_scene.set_playback_range(0, end_tick)
                set_ok = True
            except Exception:
                pass
        if not set_ok:
            try:
                ...
                try:
                    ...
                    lower.set_editor_property("Value", unreal.FrameNumber(0))
        ...
        if set_ok:
            ...
            print(f"Set sequence length to {end_frame} frames ({{fps:.0f}} fps) on {{_asset_path}}")
        else:
...
if not isinstance(asset, unreal.LevelSequence):
    ...
    if hasattr(unreal, 'LevelSequenceEditorBlueprintLibrary'):
        try:
            ...
            played = True
            print(f"Playing sequence: {_asset_path}")
        except Exception as e:
            pass
    ...
    if not played:
        try:
            ...
            print(f"Opened sequence in editor: {_asset_path}")
                ...
                if movie_scene is None:
                    ...
                    if binding is None:
                        ...
                        try:
                            ...
                            pass
                    if binding is None:
                        print("ERROR: Could not create or find binding for '" + _label + "'")
                            ...
                            if transform_track is not None:
                                ...
                                try:
                                    ...
                                    if len(channels) >= 3:
                                        ...
                                        print(f"Added transform key at frame {{frame}}: ({{x}}, {{y}}, {{z}}) for '" + _label + "'")
                                        unreal.EditorAssetLibrary.save_loaded_asset(asset)
                                    else:
...
if not isinstance(asset, unreal.LevelSequence):
    ...
    try:
        ...
        camera_actor = None
        for a in unreal.EditorLevelLibrary.get_all_level_actors():
            if a.get_actor_label() == _cam_label:
                camera_actor = a
                break
        if camera_actor is None:
            print("ERROR: Camera actor '" + _cam_label + "' not found in level")
        else:
            movie_scene = asset.get_editor_property("MovieScene")
            if movie_scene is None:
                print("ERROR: Could not access MovieScene")
                ...
                if camera_cut_track is not None:
                    try:
                        ...
                        try:
                            ...
                            pass
                        unreal.EditorAssetLibrary.save_loaded_asset(asset)
                        print(f"Added camera cut for '" + _cam_label + f"' starting at frame {{start_frame}} in {{_asset_path}}")
                    except Exception as e:
...
if not isinstance(asset, unreal.LevelSequence):
    ...
    try:
        ...
        if movie_scene is None:
            ...
            if set_ok:
                unreal.EditorAssetLibrary.save_loaded_asset(asset)
                print(f"Set playback range: frames {{start_frame}} - {{end_frame}} ({{fps:.0f}} fps) on {{_asset_path}}")
            else:
                print("ERROR: Could not set playback range via any available method")
